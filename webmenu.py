...
def normalize_panel_config(raw_config):
    ...
    if not isinstance(raw_service_access, dict):
        raw_service_access = {}
    for service in normalized["service_access"]:
        value = raw_service_access.get(service, True)
        if isinstance(value, bool):
            normalized["service_access"][service] = value
        elif isinstance(value, (int, float)):
            normalized["service_access"][service] = bool(value)
        else:
            normalized["service_access"][service] = str(value or "").strip().lower() not in {"0", "false", "no", "off", "disabled"}
    try:
...
def get_service_creation_access():
    access = load_config().get("service_access", {})
    return {
        service: bool(access.get(service, True))
        for service in CREATE_EXPIRY_DEFAULTS
    }


def is_service_creation_enabled(service):
    return bool(get_service_creation_access().get(service, True))


def save_service_creation_access(access_map):
    normalized = {
        service: bool((access_map or {}).get(service, True))
        for service in CREATE_EXPIRY_DEFAULTS
    }
    with state_lock:
        config = load_config()
        config["service_access"] = normalized
...
.result-copy-btn{position:relative;overflow:hidden;transition:border-color .18s ease,color .18s ease,background .18s ease,transform .18s ease,box-shadow .18s ease;}
.result-copy-btn:hover,.result-copy-btn:focus{transform:none!important;box-shadow:none!important;background:rgba(255,255,255,.96)!important;}
.result-copy-btn::after{content:"";position:absolute;inset:0;background:radial-gradient(circle at center,rgba(143,23,48,.16) 0%,rgba(143,23,48,0) 68%);opacity:0;transform:scale(.72);pointer-events:none;}
.result-copy-btn.is-copying::after,.result-copy-btn.is-copied::after{animation:copyRipple .55s ease;}
.result-copy-btn.is-copying{transform:scale(.985)!important;}
.result-copy-btn.is-copied{border-color:var(--success)!important;color:var(--success)!important;background:linear-gradient(180deg,#ffffff 0%,#ffeef2 100%)!important;animation:copyPulse .55s ease;}
.result-copy-btn.is-copy-failed{border-color:var(--error)!important;color:var(--error)!important;background:linear-gradient(180deg,#fff7f8 0%,#ffe7ea 100%)!important;animation:copyShake .35s ease;}
.result-copy-btn .result-copy-meta{display:inline-flex;align-items:center;gap:7px;white-space:nowrap;font-size:.82rem;font-weight:800;color:var(--text-muted);transition:color .18s ease,transform .18s ease;}
.result-copy-btn .result-copy-icon{transition:transform .18s ease,color .18s ease;}
.result-copy-btn.is-copying .result-copy-meta{transform:translateY(-1px);}
.result-copy-btn.is-copied .result-copy-meta,.result-copy-btn.is-copied .result-copy-icon{color:var(--success);}
.result-copy-btn.is-copied .result-copy-icon{animation:copyPop .42s ease;}
.result-copy-btn.is-copy-failed .result-copy-meta,.result-copy-btn.is-copy-failed .result-copy-icon{color:var(--error);}
@keyframes copyPulse{0%{transform:scale(.98)}45%{transform:scale(1.02)}100%{transform:scale(1)}}
@keyframes copyPop{0%{transform:scale(.65) rotate(-14deg);opacity:.45}70%{transform:scale(1.18) rotate(0deg);opacity:1}100%{transform:scale(1);opacity:1}}
@keyframes copyRipple{0%{opacity:0;transform:scale(.72)}35%{opacity:1}100%{opacity:0;transform:scale(1.2)}}
@keyframes copyShake{0%{transform:translateX(0)}30%{transform:translateX(-3px)}60%{transform:translateX(3px)}100%{transform:translateX(0)}}
...
.footer-support-row{display:grid;grid-template-columns:repeat(2,minmax(0,220px));gap:12px;align-items:stretch;justify-content:flex-start;margin-top:1rem;max-width:452px;}
...
def build_service_cards():
    ...
    for service, label, icon in SERVICE_META:
        ...
        href_attr = f'href="/{service}"' if button_active else 'href="#"'
        anchor_attr = "" if button_active else 'onclick="return false;" aria-disabled="true"'
        button_attr = "" if button_active else "disabled"
        ...
        badge_html = f'<span style="background:var(--primary-color);padding:4px 8px;border-radius:999px;font-weight:700;color:#fff;border:2px solid var(--ink);box-shadow:2px 2px 0 var(--ink);">Today {service_created_today}/{daily_limit}</span>'
        if enabled and not service_enabled:
            badge_html = '<span style="background:rgba(239,68,68,.12);padding:4px 8px;border-radius:999px;font-weight:700;color:var(--error);border:2px solid rgba(127,29,29,.28);box-shadow:none;">Disabled</span>'
        cards.append(
            f"""
<div class="create-cell">
  <a {href_attr} {anchor_attr}>
...
def service_creation_disabled_message(service):
    return f"{service_label(service)} creation is currently disabled in this panel by the admin."


def render_service_disabled(service, error=None):
    ...
    message = error or service_creation_disabled_message(service)
    content = f"""
<div class="container"><div class="neo-box" style="max-width:500px;margin:0 auto;text-align:center;">
  <div style="display:flex;align-items:center;justify-content:center;gap:.8em;margin-bottom:1.5em;">
  ...
  <div class="success-msg" style="background:rgba(239,68,68,.08);border-left-color:var(--warning);margin-top:.6rem;">
    <i class="fa-solid fa-ban" style="color:var(--warning);"></i>
    <div>{html.escape(message)}</div>
  </div>
  <div style="color:var(--text-secondary);margin-top:.3rem;">This only blocks account creation in the web panel. Backend services remain unchanged.</div>
...
def render_service_form(service, error=None, values=None):
    ...
    if selection_redirect:
        return selection_redirect
    if not is_service_creation_enabled(service):
        return render_service_disabled(service, error=error)
    values = values or {}
    label = service_label(service)
    icon = service_icon(service)
...
def render_service_result(service, result):
  ...
  function copyResultField(button) {
    ...
    if (!value) return;
    const statusNode = button.querySelector('[data-copy-status]');
    const iconNode = button.querySelector('[data-copy-icon]');
    const restoreState = function() {
      button.classList.remove('is-copying', 'is-copied', 'is-copy-failed');
      if (statusNode) statusNode.textContent = 'Copy';
      if (iconNode) {
        iconNode.classList.remove('fa-solid', 'fa-check', 'fa-triangle-exclamation');
        iconNode.classList.add('fa-regular');
        iconNode.classList.add('fa-copy');
      }
    };
    ...
    const flashSuccess = function() {
      button.classList.remove('is-copying', 'is-copy-failed');
      button.classList.add('is-copied');
      if (statusNode) statusNode.textContent = 'Copied!';
      if (iconNode) {
        iconNode.classList.remove('fa-regular', 'fa-copy', 'fa-triangle-exclamation');
        iconNode.classList.add('fa-solid');
        iconNode.classList.add('fa-check');
      }
      button._copyResetTimer = setTimeout(restoreState, 1600);
    };
    const flashFailure = function() {
      button.classList.remove('is-copying', 'is-copied');
      button.classList.add('is-copy-failed');
      if (statusNode) statusNode.textContent = 'Failed';
      if (iconNode) {
        iconNode.classList.remove('fa-regular', 'fa-copy', 'fa-check');
        iconNode.classList.add('fa-solid');
        iconNode.classList.add('fa-triangle-exclamation');
      }
      button._copyResetTimer = setTimeout(restoreState, 1600);
    };
    ...
    def render_copy_value(value, empty_text):
        ...
        return (
            ...
            f'<span class="result-copy-meta">'
            f'<span data-copy-status>Copy</span>'
            f'<i class="fa-regular fa-copy result-copy-icon" data-copy-icon aria-hidden="true" style="flex:none;margin-top:2px;"></i>'
            f'</span>'
            f'</button>'
        )
...
<div class="container"><div class="neo-box" style="max-width:620px;margin:0 auto;">
  ...
  <form method="POST" action="/admin"><input type="hidden" name="action" value="login">
    ...
    service_access = get_service_creation_access()
...
(function(){
  ...
  setTimeout(loadAccountManager, 0);
    ...
    protocol_access_controls = "".join(
        f"""
<div class="form-group" style="margin:0;align-items:flex-start;text-align:left;">
  <label class="form-label" style="margin-bottom:.5rem;text-align:left;max-width:none;">
    <span style="display:inline-flex;align-items:center;gap:8px;"><img src="{html.escape(icon, quote=True)}" style="height:1.05em;"> {html.escape(label)}</span>
  </label>
  <div class="form-input-container" style="justify-content:flex-start;max-width:none;">
    <select name="service_access_{html.escape(service, quote=True)}" style="max-width:none;">
      <option value="enabled"{" selected" if service_access.get(service, True) else ""}>Enabled</option>
      <option value="disabled"{" selected" if not service_access.get(service, True) else ""}>Disabled</option>
    </select>
  </div>
</div>"""
        for service, label, icon in SERVICE_META
...
<div class="container" style="max-width:1120px;">
  <div class="neo-box">{banner}
    ...
    <div class="link-box" style="margin-top:1.2rem;">
      <div style="font-weight:700;margin-bottom:.6rem;">Protocol Access</div>
      <div style="color:var(--text-secondary);margin-bottom:.9rem;">Disable or enable protocols from this hosting panel only. Disabled protocols cannot create accounts here, but the backend services themselves are not changed.</div>
      <form method="POST" action="/admin" style="margin-bottom:0;align-items:stretch;">
        <input type="hidden" name="action" value="update_service_access">
        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:14px;">
          {protocol_access_controls}
        </div>
        <button type="submit" style="width:100%;max-width:400px;margin-top:1rem;"><i class="fa-solid fa-toggle-on"></i> Save Protocol Access</button>
      </form>
    </div>
    {account_manager_html}
    {bypass_editor_html}
...
def submit_service_request(service):
    ...
    if selection_redirect:
        return selection_redirect
    if not is_service_creation_enabled(service):
        return render_service_disabled(service)
...
def admin_post():
    ...
    if action == "update_service_access":
        access_map = {}
        for service, _label, _icon in SERVICE_META:
            access_map[service] = request.form.get(f"service_access_{service}", "enabled").strip().lower() != "disabled"
        ok, saved_access = save_service_creation_access(access_map)
        if ok:
            log_admin_event("update_service_access", "success", {"service_access": saved_access})
            return redirect("/admin?success=" + urllib.parse.quote("Protocol access updated for this panel."), code=303)
        return redirect("/admin?error=" + urllib.parse.quote("Failed to update protocol access."), code=303)
